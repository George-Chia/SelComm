...
from model.utils2_6 import get_init_pose, get_goal_point
...
class StageWorld():
    ...
    def generate_goal_point(self):
        ...
        # [x, y, theta] = self.get_self_stateGT()
        # self.pre_distance = np.sqrt((self.goal_point[0] - x) ** 2 + (self.goal_point[1] - y) ** 2)
    ...
    def get_reward_and_terminate(self, t):
        ...
        reward_g = (self.pre_distance - self.distance) * 1.5
        reward_c = 0
        reward_w = 0
        result = 0
    ...
    def reset_pose(self):
        ...
        while np.abs(reset_pose[0] - x_robot) > 0.2 or np.abs(reset_pose[1] - y_robot) > 0.2:  # this loop ???
            [x_robot, y_robot, theta] = self.get_self_stateGT()
        rospy.sleep(0.05)
